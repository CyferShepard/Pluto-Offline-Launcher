...
namespace Pluto_Offline_Launcher
{
    public partial class Launcher : Form
    {
        ...
        private string plutolauncherlink = "https://fastcache.plutonium.pw/r372.zip";
   ...
   MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public void set_button_state(bool enable)
        {
            if (enable)
            {
                checkgame();
...
MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void file_cleanup()
        {
            try
            {
                ...
                if (File.Exists(Path.Combine(loc, "ZipExtractor.exe")))
                {
        ...
        private void btn_MP_Click(object sender, EventArgs e)
        {
            if (usernameExists())
            {
                ...
                var process = Process.Start(gameloc + "t6rmp.exe");
                process.WaitForExit();
    ...
    MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btn_ZM_Click(object sender, EventArgs e)
        {
            ...
            {
                set_button_state(false);
                var process = Process.Start(gameloc + "t6rzm.exe");
                process.WaitForExit();
                set_button_state(true);
            }
            else
